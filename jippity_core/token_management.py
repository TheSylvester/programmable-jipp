...
from models.llm_models import Message, MessageContent
from llm_providers.llm_selector import get_model_context_window
    ...
    if "gpt" in model:
        ...
        if tokenizer_model is None:
            ...
            estimated_token_count = (
                len(text) // 3
            )  # Estimate: average 1 token per 3 characters            tokens = encoding.encode(text)
            return len(estimated_token_count)
...
def main():
    ...
    # Example usage with the modified Message class structure
    message_1 = Message(role="user", content="Hello, how are you?")
    message_2 = Message(
        role="assistant",
        content=[
            MessageContent(type="text", text="I'm fine, thank you!"),
            MessageContent(
                type="image_url", image_url={"url": "http://example.com/image.png"}
            ),
        ],
    )
    message_3 = Message(role="system", content="You are a helpful assistant.")

    # Testing the function
    print(get_text_from_message(message_1))  # Output: "Hello, how are you?"
    print(get_text_from_message(message_2))  # Output: "I'm fine, thank you!"
    print(get_text_from_message(message_3))  # Output: "You are a helpful assistant."
    ...
    messages = [
        ...
        Message(
            role="assistant",
            content=[MessageContent(type="text", text="I'm fine, thank you!")],
        ),
        Message(role="user", content="Tell me about the history of AI."),
    ]

    context_window = 50  # Define a context window size
    trimmed_messages = trim_messages_to_context_window(messages, context_window)
